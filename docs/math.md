...
TeX 里 `#` 是宏参数符、`%` 是注释符，MathJax 遇到它们不会报错退出，而是输出
`<merror>`——一个用字形色填充的整行 `<rect>`，在文章里就是**一条黑块**。
所以插件在交给 MathJax 前会把"裸"的 `#` `%` 重新转义（已有反斜杠的不动）。

**2. 不要把 LaTeX 源码放进 `alt`。**

`$n>1$` 的源码里含 `>`，写进 `alt="n>1"` 后，公众号编辑器的解析器会在 `>` 处
提前结束 `<img>` 标签，剩下的 `1" width="44.05" ...>` 会作为**可见文本**漏到正文里。
现在插件不输出 `alt`——标签体内任何位置都不能出现裸 `<` / `>`。

其他防御：渲染结果里出现 `data-mjx-error` 时视为失败，保留源码文本并在 stderr
打印告警，不让黑块进文章，也不让单个坏公式中断整篇渲染。
